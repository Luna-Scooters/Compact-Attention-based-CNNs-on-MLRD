...
from tensorflow.keras.layers import Activation, BatchNormalization, Conv2D, Dense, Dropout, Flatten, MaxPool2D, Input, DepthwiseConv2D, ReLU, AvgPool2D,GlobalAveragePooling2D, Concatenate, Add, GlobalAvgPool2D, Reshape, Permute, Lambda, GlobalMaxPool2D, GlobalMaxPooling2D, multiply
...
# ========================================================= Squeeze and Excitation Block Start =================================================================
def _se_block(input_tensor, ratio=16):
    ...
    return x
# ========================================================= Squeeze and Excitation Block End =================================================================
...
def _channel_attention(input_feature, ratio=8):
    ...
    shared_layer_two = Dense(channel,
                            ...
                            use_bias=True,
                            bias_initializer='zeros')
    # shared_layer_three = Dense(channel,
    #                         kernel_initializer='he_normal',
    #                         use_bias=True,
    #                         bias_initializer='zeros')
    ...
    # assert avg_pool.shape[1:] == (1,1,channel//(ratio/2))
...
def _spatial_attention(input_feature):
    ...
    # cbam_feature1 = Conv2D(filters = 8,
    ...
    # cbam_feature2 = Conv2D(filters = 4,
    #                 kernel_size=kernel_size,
    #                 strides=1,
    #                 padding='same',
    #                 activation='sigmoid',
    #                 kernel_initializer='he_normal',
    #                 use_bias=False)(cbam_feature1)

    # cbam_feature3 = Conv2D(filters = 2,
    #                 kernel_size=kernel_size,
    #                 strides=1,
    #                 padding='same',
    #                 activation='sigmoid',
    #                 kernel_initializer='he_normal',
    #                 use_bias=False)(cbam_feature2)
...
def CoordAtt(x, reduction=32):
    ...
    [b, h, w, c] = x_shape
    x_h = layers.AveragePooling2D(pool_size=(1, w), strides=1)(x)
    x_w = layers.AveragePooling2D(pool_size=(h, 1), strides=1)(x)
    x_w = tf.transpose(x_w, [0, 2, 1, 3])

    y = tf.concat([x_h, x_w], axis=3)
    mip = max(8, c // reduction)

    y = layers.Conv2D(mip, (1, 1), strides=1, padding='valid', activation=None)(y)
    y = layers.BatchNormalization()(y)
    y = coord_act(y)

    x_h, x_w = tf.split(y, num_or_size_splits=2, axis=3)
    x_w = tf.transpose(x_w, [0, 2, 1, 3])

    a_h = layers.Conv2D(c, (1, 1), strides=1, padding='valid', activation='sigmoid')(x_h)
    a_w = layers.Conv2D(c, (1, 1), strides=1, padding='valid', activation='sigmoid')(x_w)
