...
class MobileNetV3_Small(MobileNetBase):
    ...
    def build(self, plot=False):
        ...
        x = self._conv_block(inputs, 16, (3, 3), strides=(2, 2), nl='HS')   #Adjusted number of channels preserving the original layer-channel ratio and to suffice deployment needs
...
# def mobilenetv3_0_25_transfer_learning():
...
# #     FC = Dense(num_classes, activation=None, name='output_layer')(GAP)    
    
#     final_model = Model(inputs=model.inputs, outputs=FC)
#     return final_model

# fp32_model = mobilenetv3_0_25_transfer_learning()
# print(fp32_model.summary())
...
tf.keras.utils.plot_model(
    fp32_model,
    to_file='wandb-MNV3-CA-cropped-0.1.png',
    show_shapes=True,
    show_dtype=True,
...
folder_path = 'Models/wandb-MNV3-CA-cropped-0.1-224-224'
...
checkpoint = tf.keras.callbacks.ModelCheckpoint('Models/wandb-MNV3-CA-cropped-0.25-224-224/wandb-MNV3-CA-0.1-cropped.h5', 
                                                monitor='val_loss', 
                                                mode='min', 
                                                verbose=1, 
                                                save_best_only=True)
